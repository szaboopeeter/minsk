...
using static Minsk.CodeAnalysis.Symbols.FunctionSymbol;
...
namespace Minsk.CodeAnalysis.Binding
{
    internal sealed class Binder
    {
        ...
        public static BoundGlobalScope BindGlobalScope(bool isScript, BoundGlobalScope previous, ImmutableArray<SyntaxTree> syntaxTrees)
        {
            ...
            if (isScript && globalStatements.Any())
            {
        ...
        private BoundStatement BindStatement(StatementSyntax syntax, bool isGlobal = false)
        {
            ...
            if ((!_isScript || !isGlobal) && result is BoundExpressionStatement bes)
            {
                var isAllowedExpression = bes.Expression.Kind == BoundNodeKind.AssignmentExpression ||
                bes.Expression.Kind == BoundNodeKind.CallExpression ||
                bes.Expression.Kind == BoundNodeKind.ErrorExpression;
        ...
        private BoundExpression BindConversion(TextLocation location, BoundExpression expression, TypeSymbol type, bool allowExplicit = false)
        {
            ...
            {
                _diagnostics.ReportCannotConvertImplicitly(location, expression.Type, type);
            }
